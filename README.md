...
* [loop54-core-2.0.0.jar](https://static.loop54.com/lib/java/core/loop54-core-2.0.0.jar) - **Core** functionality **without** its dependencies
* [loop54-core-all-2.0.0.jar](https://static.loop54.com/lib/java/core/loop54-core-all-2.0.0.jar) - **Core** functionality **with** all its dependencies 
* [loop54-spring-2.0.0.jar](https://static.loop54.com/lib/java/spring/loop54-spring-2.0.0.jar) - **Core** functionality **plus Spring** specific implementation **without** its dependencies
* [loop54-spring-all-2.0.0.jar](https://static.loop54.com/lib/java/spring/loop54-spring-all-2.0.0.jar) - **Core** functionality **plus Spring** specific implementation **with** all its dependencies

Or download the source and reference the core and spring modules and build it using gradle.

Requires Java 17 or later. If running Java 1.8 or under or using javax instead of jakarta please use an older version of the library available in the 
GitHub history.
...
_Building, generating javadoc and running the test apps requires **JDK 17**._ 
...
If you're on windows, download the 17 JDK from https://www.java.com/
...
This will download gradle 9.3 and all dependencies, build all modules and run the unit tests. 
...
If you get an error saying `Could not set unknown property '[x]'` for the `web` or `codeexamples` projects, that means that `gretty` plugin is not compatible with your gradle version. You need to either upgrade the gradle wrapper or specify a compatible `gretty` version in the `build.gradle` 
config files in the `web` and `codeexamples` projects.
